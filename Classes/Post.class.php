...
class Post extends MysqliConnect{
    private $title , $videoLink , $description , $image , $category , $type , $id;
    
    public function setPostValue($title , $videoLink , $description , $image , $category , $type , $id = null){
    ...
    private function addNewPost(){
        ...
        $this->insert("post", "`user_id`, `title`, `video`, `desc`, `image`, `category` ,`link`",
                    "'$user_id','$this->title','$this->videoLink','$this->description','$this->image','$this->category' , '$link'"
                );
        if($this->execute()){
    ...
    private function editPost(){
        if($this->image == NULL){
            $this->update('post', "`title`='$this->title',`video`='$this->videoLink',`desc`='$this->description',`category`='$this->category'", 'id', $this->id);
            if($this->execute()){
                echo Messages::setMsg('تم', 'تحديث الفيديو بنجاح', 'success') . Messages::getMsg();
            }
        }else{
            $this->update('post', "`title`='$this->title',`video`='$this->videoLink',`image`='$this->image',`desc`='$this->description',`category`='$this->category'", 'id', $this->id);
            if($this->execute()){
                echo Messages::setMsg('تم', 'تحديث الفيديو بنجاح', 'success') . Messages::getMsg();
            }
        }
